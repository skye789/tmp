...
def torch_old_fft(input, signal_ndim, normalized=True):
    ...
    if signal_ndim == 1:
        output = torch.fft.fftn(input, dim=(-1), norm=norm)
    elif signal_ndim == 2:
        output = torch.fft.fftn(input, dim=(-2, -1), norm=norm)
    output = torch.view_as_real(output)
    return output

def torch_old_ifft(input, signal_ndim, normalized=True):
    if normalized:
        norm = "ortho"
    else:
        norm = "backward"
    input = torch.view_as_complex(input.contiguous())
    if signal_ndim == 1:
        output = torch.fft.ifftn(input, dim=(-1), norm=norm)
    elif signal_ndim == 2:
        output = torch.fft.ifftn(input, dim=(-2, -1), norm=norm)
    output = torch.view_as_real(output)
    return output
...
class DataConsistencyInKspace(nn.Module):
    ...
    Warning: note that FFT2 (by the default of torch_old_fft) is applied to the last 2 axes of the input.
    This method detects if the input tensor is 4-dim (2D data) or 5-dim (3D data)
    and applies FFT2 to the (nx, ny) axis.
    DataConsistency is DC term(xf, where f=0)
    """
    ...
    def perform(self, x, k0, mask):
        ...
        k = torch_old_fft(x, 2, normalized=self.normalized)
        out = data_consistency(k, k0, mask, self.noise_lvl)
        x_res = torch_old_ifft(out, 2, normalized=self.normalized)
...
class TransformDataInXfSpaceTA(nn.Module):
    ...
    def perform(self, x, k0, mask):
        ...
        x = x.permute(0, 4, 2, 3, 1)  #[nb, nt, nx, ny, 2]
        mask = mask.permute(0, 4, 2, 3, 1)
        k0 = k0.permute(0, 4, 2, 3, 1)
        k = torch_old_fft(x, 2, normalized=self.normalized)
        if self.divide_by_n:
        ...
        x_diff = torch_old_ifft(k_diff, 2, normalized=self.normalized)

        # transform to x-f space to get the baseline
        k_avg = data_consistency(k_avg, k0, mask)
        x_avg = torch_old_ifft(k_avg, 2, normalized=self.normalized)

        x_avg = x_avg.permute(0, 2, 3, 1, 4)  # [n, nx, ny, nt, 2]
        x_f_avg = fftshift_pytorch(torch_old_fft(ifftshift_pytorch(x_avg, axes=[-2]), 1, normalized=self.normalized), axes=[-2])
        x_f_avg = x_f_avg.permute(0, 4, 1, 2, 3)

        # difference data
        x_diff = x_diff.permute(0, 2, 3, 1, 4)  # [n, nx, ny, nt, 2]
        x_f_diff = fftshift_pytorch(torch_old_fft(ifftshift_pytorch(x_diff, axes=[-2]), 1, normalized=self.normalized), axes=[-2])
...
class TransformDataInXtSpaceTA_mc(nn.Module):
    ...
    def perform(self, x, k0, mask, sensitivity):
        ...
        x_avg = torch_old_ifft(k_avg, 2, normalized=True)
...
class TransformDataInXfSpaceTA_mc(nn.Module):
    ...
    def perform(self, x, k0, mask, sensitivity):
        ...
        x_f_avg = fftshift_pytorch(torch_old_fft(ifftshift_pytorch(Sx_avg, axes=[-2]), 1, normalized=self.normalized), axes=[-2])
        x_f_avg = x_f_avg.permute(2, 0, 1, 3)

        # difference data
        Sx_diff = Sx_diff.permute(1, 2, 0, 3)  # [nx, ny, nt, 2]
        x_f_diff = fftshift_pytorch(torch_old_fft(ifftshift_pytorch(Sx_diff, axes=[-2]), 1, normalized=self.normalized), axes=[-2])
